...
Downloads actual Instagram content using modern techniques
"""
...
class RealInstagramScraper:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('real_instagram_scraper.log'),
    ...
    def get_profile_data(self, username):
        """Get profile data using modern Instagram structure"""
        try:
            ...
            for script in scripts:
                if script.string:
                    ...
                    patterns = [
                        r'"profilePage":\[{"user":(.*?)}',
                        r'"user":(.*?)},"edge_owner_to_timeline_media"',
                        r'"user_id":"(\d+)"',
                        r'"full_name":"([^"]+)"',
                    ...
                    for pattern in patterns:
                        ...
                        if matches:
                            if 'user_id' in pattern:
                                profile_data['user_id'] = matches[0]
                            elif 'full_name' in pattern:
                                profile_data['full_name'] = matches[0]
                            elif 'edge_followed_by' in pattern:
                                profile_data['followers'] = matches[0]
                            elif 'edge_owner_to_timeline_media' in pattern:
                                profile_data['posts_count'] = matches[0]
    ...
    def get_posts_data(self, username, max_posts=6):
        """Get posts data using modern Instagram structure"""
        try:
            profile_url = f"https://www.instagram.com/{username}/"
            ...
            for script in scripts:
                if script.string and 'edge_owner_to_timeline_media' in script.string:
                    try:
                        # Extract posts data using regex
                        posts_pattern = r'"edge_owner_to_timeline_media":\{"edges":\[(.*?)\],'
                        posts_match = re.search(posts_pattern, script.string)
                        
                        if posts_match:
                            posts_json = '[' + posts_match.group(1) + ']'
                            ...
                            for i, post in enumerate(posts[:max_posts]):
                                try:
                                    ...
                                    self.logger.error(f"Error parsing post {i}: {e}")
                                    continue
    ...
    def scrape_brand_instagram(self, brand_name, instagram_url):
        ...
        try:
            ...
            # Get posts data
            posts_data = self.get_posts_data(username, max_posts=6)
    ...
    def run(self, brands_file="downloads/brands-list.md", max_brands=None):
        ...
        self.logger.info("Starting Real Instagram Scraper...")
...
def main():
    """Main function to run the real Instagram scraper"""
